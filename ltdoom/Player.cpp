...
void idPlayer::GetViewPos( idVec3 &origin, idMat3 &axis ) const {
	...
	if ( health <= 0 ) {
		...
		idAngles ltang;
		idVec3 ltpos;
		linuxtrack_data_t ltdata;

		if(linuxtrack_get_pose(&ltdata.h, &ltdata.p ,&ltdata.r, &ltdata.x, &ltdata.y, &ltdata.z, &ltdata.counter)) {
			ltang.yaw = ltdata.h;
			ltang.pitch = ltdata.p * -1;
			ltang.roll = ltdata.r;
		}
		origin = GetEyePosition() + viewBob;
		angles = viewAngles + viewBobAngles + ltang + playerView.AngleOffset();
